...
function build::notification-controller::create_tarball() {
  platform=${1}
...
function build::notification-controller::tarball() {
  build::common::ensure_tar
  mkdir -p "$TAR_PATH"
  build::notification-controller::create_tarball "linux/amd64"
  rm -rf $BIN_ROOT
  rm -rf $LICENSES_PATH
}

build::notification-controller::tarball

...
function build::helm-controller::create_tarball() {
  platform=${1}
...
function build::helm-controller::tarball() {
  build::common::ensure_tar
  mkdir -p "$TAR_PATH"
  build::helm-controller::create_tarball "linux/amd64"
  rm -rf $BIN_ROOT
  rm -rf $LICENSES_PATH
}

build::helm-controller::tarball

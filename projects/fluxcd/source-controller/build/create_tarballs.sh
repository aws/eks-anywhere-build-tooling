...
function build::source-controller::create_tarball() {
  platform=${1}
...
function build::source-controller::tarball() {
  build::common::ensure_tar
  mkdir -p "$TAR_PATH"
  build::source-controller::create_tarball "linux/amd64"
  rm -rf $BIN_ROOT
  rm -rf $LICENSES_PATH
}

build::source-controller::tarball

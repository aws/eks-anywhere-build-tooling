...
function build::etcdadm-bootstrap-provider::create_tarball() {
  platform=${1}
...
function build::etcdadm-bootstrap-provider::tarball() {
  build::common::ensure_tar
  mkdir -p "$TAR_PATH"
  build::etcdadm-bootstrap-provider::create_tarball "linux/amd64"
  rm -rf $BIN_ROOT
  rm -rf $LICENSES_PATH
}

build::etcdadm-bootstrap-provider::tarball

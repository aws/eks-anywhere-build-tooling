...
TAR_PATH="${4?Specify fourth argument - tarball output path}"
...
function build::cloud-provider-vsphere::create_tarball() {
  platform=${1}
...
function build::cloud-provider-vsphere::tarball() {
  build::common::ensure_tar
  mkdir -p "$TAR_PATH"
  build::cloud-provider-vsphere::create_tarball "linux/amd64"
  rm -rf $BIN_ROOT
  rm -rf $LICENSES_PATH
}

build::cloud-provider-vsphere::tarball

...
function build::local-path-provisioner::create_tarball() {
  platform=${1}
...
function build::local-path-provisioner::tarball() {
  build::common::ensure_tar
  mkdir -p "$TAR_PATH"
  build::local-path-provisioner::create_tarball "linux/amd64"
  rm -rf $BIN_ROOT
  rm -rf $LICENSES_PATH
}

build::local-path-provisioner::tarball

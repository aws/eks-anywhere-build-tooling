...
func TestGetLocalApiBindPortFromJoinConfigYaml(t *testing.T) {
...
controlPlane:
  localAPIEndpoint:
    ...
    bindPort: 443
`
		port, _ := getLocalApiBindPortFromJoinConfigYaml(yamlWithMultipleDoc)
		if port != 443 {
			t.Errorf("Returned unexpected port, expected: %d, actual: %d", 443, port)
		}
	})
...
controlPlane:
  localAPIEndpoint:
    ...
    bindPort: 443
`
		port, _ := getLocalApiBindPortFromJoinConfigYaml(yamlWithSingleDoc)
		if port != 443 {
			t.Errorf("Returned unexpected port, expected: %d, actual: %d", 443, port)
		}
	})

	t.Run("yamlWithoutJoinConfigurationKind", func(t *testing.T) {
		const yamlWithoutJoinConfigurationKind = `
apiVersion: kubeadm.k8s.io/v1beta2
kind: OtherConfiguration
controlPlane:
  localAPIEndpoint:
    ...
    bindPort: 443
`
		_, err := getLocalApiBindPortFromJoinConfigYaml(yamlWithoutJoinConfigurationKind)
		if err == nil {
			t.Errorf("Should return error for invalid yaml")
		}
	})
	...
	t.Run("emptyYaml", func(t *testing.T) {
		_, err := getLocalApiBindPortFromJoinConfigYaml("")
		if err == nil {
			t.Errorf("Should return error for empty yaml")
		}
	})
	...
	t.Run("invalidYaml", func(t *testing.T) {
		_, err := getLocalApiBindPortFromJoinConfigYaml("SomeRandomText")
		if err == nil {
			t.Errorf("Should return error for invalid yaml")
		}
	})

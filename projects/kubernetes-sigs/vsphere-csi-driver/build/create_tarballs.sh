...
function build::vsphere-csi-driver::create_tarball() {
  platform=${1}
...
function build::vsphere-csi-driver::tarball() {
  build::common::ensure_tar
  mkdir -p "$TAR_PATH"
  build::vsphere-csi-driver::create_tarball "linux/amd64"
  rm -rf $BIN_ROOT
  rm -rf $LICENSES_PATH
}

build::vsphere-csi-driver::tarball

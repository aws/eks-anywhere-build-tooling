...
if [ -n "$ANSIBLE_VERBOSITY" ]; then
    ...
    build::jq::update_in_place ${MAKE_ROOT}/${IMAGE_BUILDER_DIR}/packer/$HYPERVISOR_DIRNAME/$PACKER_FILE_NAME "(.provisioners[] | select(.type == \"ansible\").extra_arguments) += [\"$ANSIBLE_VERBOSITY\"]"
fi
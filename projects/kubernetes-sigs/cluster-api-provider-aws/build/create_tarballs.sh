...
function build::cluster-api-provider-aws::create_tarball() {
  platform=${1}
...
function build::cluster-api-provider-aws::tarball() {
  build::common::ensure_tar
  mkdir -p "$TAR_PATH"
  build::cluster-api-provider-aws::create_tarball "linux/amd64"
  rm -rf $BIN_ROOT
  rm -rf $LICENSES_PATH
}

build::cluster-api-provider-aws::tarball

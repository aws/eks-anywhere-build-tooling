...
var individualTestLogFileTemplate = "%s-%s-%s"
...
func (l *testLogFetcher) FetchLogs(opts ...FetchLogsOpt) error {
	...
	failedTests, err := l.GetBuildProjectLogs()
	if err != nil {
		return err
	}
...
func (l *testLogFetcher) GetBuildProjectLogs() (failed []testResult, err error) {
	logger.Info("Fetching build project logs...")
	latestBuild := l.buildAccountCodebuildClient.FetchLatestBuildForProject()
